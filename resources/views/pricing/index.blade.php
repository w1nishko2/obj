    ...
    <div class="pricing-page">
        ...
        <div class="row g-3 justify-content-center">
            ...
            <div class="col-md-6 col-lg-4">
                <div class="pricing-card {{ $plan->slug === 'yearly' ? 'pricing-card-best' : '' }}">
                    @if($plan->slug === 'yearly')
                    <div class="pricing-label">Экономия 25%</div>
                    @endif
                    
                    <div class="pricing-header">
                        <div class="pricing-badge {{ $plan->slug === 'free' ? 'pricing-badge-free' : ($plan->slug === 'yearly' ? 'pricing-badge-best' : '') }}">
                            @if($plan->slug === 'free')
                                <i class="bi bi-gift"></i> Дешёвый вход
                            @else
                                <i class="bi bi-star{{ $plan->slug === 'yearly' ? '-fill' : '' }}"></i> Статус Прораба
                            @endif
                        </div>
                        <h3>{{ $plan->name }}</h3>
                        <div class="pricing-price">
                            <span class="price">{{ number_format($plan->price, 0, ',', ' ') }} ₽</span>
                            <span class="period">/{{ $plan->duration_days ? ($plan->duration_days === 365 ? 'год' : 'месяц') : 'навсегда' }}</span>
                        </div>
                        @if($plan->slug === 'yearly')
                    ...
                    @auth
                        ...
                        @if($hasActivePlan)
                            <button class="pricing-btn {{ $plan->slug === 'yearly' ? 'pricing-btn-best' : '' }} {{ $plan->slug === 'free' ? 'pricing-btn-free' : '' }}" disabled style="opacity: 0.6; cursor: not-allowed;">
                        ...
                        @else
                            <form action="{{ route('payment.create') }}" method="POST">
                                ...
                                <button type="submit" class="pricing-btn {{ $plan->slug === 'yearly' ? 'pricing-btn-best' : '' }} {{ $plan->slug === 'free' ? 'pricing-btn-free' : '' }}">
                                    <i class="bi bi-credit-card"></i> {{ $plan->slug === 'free' ? 'Активировать бесплатно' : 'Оформить подписку' }}
                                </button>
                            </form>
                            <small class="pricing-note">
                        ...
                        @endif
                    @else
                        <button class="pricing-btn {{ $plan->slug === 'yearly' ? 'pricing-btn-best' : '' }} {{ $plan->slug === 'free' ? 'pricing-btn-free' : '' }}" onclick="window.location.href='{{ route('login') }}'">
                            <i class="bi bi-credit-card"></i> {{ $plan->slug === 'free' ? 'Оплатить 1 ₽' : 'Оформить подписку' }}
                        </button>
                        <small class="pricing-note">
