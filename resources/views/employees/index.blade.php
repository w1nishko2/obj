            ...
            @foreach($employees as $employee)
                <div class="employee-card">
                    ...
                    <div class="employee-actions">
                        <button onclick="editEmployee({{ $employee->id }}, '{{ $employee->name }}', '{{ $employee->phone }}', '{{ addslashes($employee->description ?? '') }}')" 
                                class="minimal-btn minimal-btn-ghost minimal-btn-sm">
                            <i class="bi bi-pencil"></i>
                        </button>
                        <form action="{{ route('employees.destroy', $employee) }}" method="POST" 
                              onsubmit="return confirm('Удалить сотрудника {{ $employee->name }}?')" 
...
<div class="modal fade" id="addEmployeeModal" tabindex="-1">
    <div class="modal-dialog modal-fullscreen m-0">
        <div class="modal-content">
            <form action="{{ route('employees.store') }}" method="POST" class="d-flex flex-column h-100">
                @csrf
